// src/app/api/protected-admin/route.ts
...
const handler = async (req: NextRequest, token: string) => {
  ...
  try {
    ...
    return NextResponse.json({ message: 'This is a protected admin route', user: userData, token: token });
  } catch (err) {
    console.error(err);
...
export const GET = authMiddleware(handler, true);
